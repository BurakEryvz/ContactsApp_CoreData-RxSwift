    ...
    @IBAction func editButtonPressed(_ sender: UIBarButtonItem) {
        ...
        let doneAction = UIAlertAction(title: "Done", style: .default) { action in
            if let textfields = alert.textFields {
                if let contactName = textfields[0].text, let contactNumber = textfields[1].text {
                
                    self.viewModel.updateContact(contact: self.contact!, contact_name: contactName, contact_number: contactNumber)
